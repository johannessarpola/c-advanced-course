...
int socketReceive(int sock_fd, char *response, short response_length)
{
    int shortRetval = -1;
    struct timeval tv;
    ...
    shortRetval = recv(sock_fd, response, response_length, 0);
    printf("Response %s\n", response);
    return shortRetval;
}
...
int main(int argc, char *argv[])
{
    ...
    read_size = socketReceive(sock_fd, server_reply, 200);
